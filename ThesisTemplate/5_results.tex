...
In conclusion to answer Research Question~\ref{rq:cold}: it was found that the tested clustering approaches are not applicable in our case, but may be effective in cases where there is more data available.
...
\end{footnotesize}
\caption{\label{tab:cmnn} \footnotesize{Classification Results Neural Network}}
\end{table}
...
In conclusion to answer Research Question~\ref{rq:model}: it was found that the Neural Network has the best performance, and that all tested models displayed a tendency to over classify the positive labels.
...
In conclusion to answer Research Question~\ref{rq:strategies} the findings are that in our case the tested methods are not effective in tackling class imbalance, and that the performance improves significantly if all user features are removed.  
