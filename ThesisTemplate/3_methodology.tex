...
The advantage of the RandomForest classification algorithm is that it robust to the inclusion of irrelevant features.
...
The outcomes of the previous mentioned models can be two types of recommendation: 1) the prediction of the utility of a certain job to a user, or 2) a list of the top-\textit{n} useful jobs for a user.
...
Hereafter the probabilities in each row are ranked descending, and from that the top-\textit{n} ranks (jobs) are returned. 
